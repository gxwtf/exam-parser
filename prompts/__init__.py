...
1. 只识别这7种题型：完形填空、语法填空、阅读、七选五、选词填空、阅读表达、作文
   - 注意：并非每份试卷都包含所有题型，阅读表达、选词填空、语法填空可能不存在，不存在的题型不要强行创建
   - 作文可能有1-2个（如第一节+第二节，但是不要在作文题目内容中显示“第一节”“第二节”这样的文字），每个作文一个独立section
...
### 作文
- 有的试卷可能有2个作文（如第一节+第二节，但是不要在作文题目内容中显示“第一节”“第二节”这样的文字），每个作文一个独立section，依次排列
- 输出 start 锚点（程序自动提取题目题干内容，而不是范文答案内容）
