...
**空格规则（极其重要！）**：
- 如果空的左右两边都是文章内容（单词/字母），标签前后各加一个空格：`were <ClozeBlank></ClozeBlank> throughout`
- 如果空后面是标点符号（`.` `,` `;` `!` `?`），标签后不加空格：`XXX <ClozeBlank></ClozeBlank>.`
...
- **没有解析时自己生成**：如果MD参考答案中只有答案没有解析，则根据文章内容和答案，自己生成一段简洁的解析（中文，1-3句话），说明为什么选这个答案
